...
\usepackage{Sweave}
\usepackage{times}
\usepackage{amsmath,amsfonts,amssymb,psfrag,color,pbox,subfigure,graphicx}
\usepackage{graphicx}
\usepackage{rotating}
\usepackage{color}
\usepackage{syntonly}
...
%\SweaveOpts{engine=R,eps=FALSE,pdf=TRUE,strip.white=all,keep.source=TRUE}
%\SweaveOpts{engine=R,eps=FALSE,pdf=TRUE,keep.source=TRUE}
\SweaveOpts{engine=R,eps=FALSE,pdf=TRUE}
\SweaveOpts{prefix=TRUE,prefix.string=fig,include=FALSE}
\SweaveOpts{keep.blank.space=TRUE}



\setlength{\headheight}{15.2pt}
\pagestyle{fancyplain}
\lhead[]{ICA correction with {\sc icaOcularCorrection}}
\chead[]{}
\rhead[]{Antoine Tremblay}
\lfoot[]{}
\cfoot[]{\thepage}
\rfoot[]{}
...
described in \citeA{Flexer2005}. The process of correcting electro- and
magneto-encephalographic data (EEG/MEG) begins by running function {\tt icac},
which first performs independent components analysis (ICA) to decompose the
data frame into independent components (ICs) using function {\tt fastICA} from
the package of the same name \cite{Marchini2012}. It then calculates for each
trial the correlation between each IC and each one of the noise signals --
there can be one or more, e.g., vertical and horizontal electro-oculograms
(VEOG and HEOG), electro-myograms (EMG), electro-cardiograms (ECG), galvanic
skin responses (GSR), and other noise signals. Subsequently, portions of an IC
corresponding to trials at which the correlation between it and a noise signal
was at or above threshold \cite<set to 0.4 by default;>[p. 1001]{Flexer2005}
are either zeroed-out in the source matrix, $S$, or subtracted from the data
that was passed to function {\tt icac}. 
...
summary.icac}), the scalp topography of the ICs (using function {\tt topo.ic}),
the time courses of the ICs (using functions {\tt plot.tric} and {\tt
plot.nic}, and other diagnostic plots. Once these ICs have been identified,
...
plot.avgba} and {\tt plot.trba}. Some worked-out examples with {\tt R} code are
...
save(smry, file = "summaries/smry_egi129.rda")
#
smry  <- summary(res, ic = 52, print = FALSE)
save(smry, file = "summaries/smry_egi129_IC52.rda")
#
smry  <- summary(res, ic = 6, print = FALSE)
save(smry, file = "summaries/smry_egi129_IC6.rda")
#
smry  <- summary(res, ic = 43, print = FALSE)
save(smry, file = "summaries/smry_egi129_IC43.rda")
@
<<ShowEGI129SummaryTopTen,eval=FALSE,echo=TRUE>>=
load("summaries/smry_egi129.rda")
smry[1:10,]
@
...
<<loadEGI129resIC101,eval=FALSE,echo=TRUE>>=
load("summaries/smry_egi129_IC52.rda")
smry
@
...
pdf(file="figs/fig-IC52E14.pdf")
plot.tric(res, ic = 52, noise.sig = "E14",
...
\begin{figure}[ht]
  \centering
  \includegraphics{figs/fig-IC52E14.pdf}
...
We can also use function {\tt plot.nic} to look at an IC and a noise signal at
...
pdf("figs/fig-plotNICIC52E14.pdf")
par(mfrow = c(3, 3))
for(i in 85:93){
	plot.nic(x = res, data = egi129, ic = 52, 
...
\begin{figure}[ht]
  \centering
  \includegraphics{figs/fig-plotNICIC52E14.pdf}
...
pdf("figs/fig-topomapIC52.pdf")
topo.ic(x = res, ic = 52, coords = "egi.129") 
dev.off()
@

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.55\textwidth,height=0.4\textheight]{figs/fig-topomapIC52.pdf}
...
<<loadSummaryIC6,eval=FALSE,echo=TRUE>>=
load("summaries/smry_egi129_IC6.rda")
smry
@

IC 6 correlates mostly with channel E14 (\Sexpr{smry[smry$NoiseSignal=="E14",3]} trials). We will nevertheless look at IC 6 against noise signal
E126 (\Sexpr{smry[smry$NoiseSignal=="E126", 3]} trials).

<<plotIC6E126,eval=FALSE,echo=TRUE>>=
pdf(file="figs/fig-IC6E126.pdf")
plot.tric(res, ic = 6, noise.sig = "E126",
...
\begin{figure}[ht]
  \centering
  \includegraphics{figs/fig-IC6E126.pdf}
...
pdf("figs/fig-plotNICIC6E126.pdf")
par(mfrow = c(3, 3))
for(i in 253:261){
	plot.nic(x = res, data = egi129, ic = 6, 
...
\begin{figure}[ht]
  \centering
  \includegraphics{figs/fig-plotNICIC6E126}
...
pdf("figs/fig-topomapIC6")
topo.ic(x = res, ic = 6, coords = "egi.129") 
dev.off()
@

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.55\textwidth,height=0.4\textheight]{figs/fig-topomapIC6.pdf}
...
<<loadSummaryIC43,eval=FALSE,echo=TRUE>>=
load("summaries/smry_egi129_IC43.rda")
smry
@

IC 43 correlates mostly with channel E21 (\Sexpr{smry[smry$NoiseSignal=="E21",3]} trials). 

<<plotIC43E21,eval=FALSE,echo=TRUE>>=
pdf(file="figs/fig-IC43E21.pdf")
plot.tric(res, ic = 43, noise.sig = "E21",
...
\begin{figure}[ht]
  \centering
  \includegraphics{figs/fig-IC43E21.pdf}
...
pdf("figs/fig-plotNICIC43E21.pdf")
par(mfrow = c(3, 3))
for(i in 1:9){
	plot.nic(x = res, data = egi129, ic = 43, 
...
\begin{figure}[ht]
  \centering
  \includegraphics{figs/fig-plotNICIC43E21}
...
pdf("figs/fig-topomapIC43")
topo.ic(x = res, ic = 43, coords = "egi.129") 
dev.off()
@

\begin{figure}[ht]
  \centering
  \includegraphics[width=0.55\textwidth,height=0.4\textheight]{figs/fig-topomapIC43.pdf}
...
#
# create plot
pdf(file = "figs/fig-uncorrectedCorrected0.pdf")
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.55\textwidth,height=0.4\textheight]{figs/fig-uncorrectedCorrected0.pdf}
...
pdf(file="figs/fig-ICNumTrials3.pdf")
...
\begin{figure}[ht]
  \centering
  \includegraphics{figs/fig-ICNumTrials3.pdf}
...
#
# create plot
pdf(file = "figs/fig-uncorrectedCorrected3.pdf")
par(mfrow = c(2, 2), mar = c(5.1, 4.1, 4.1, 2.1))
# plot waveforms
plot(time, avg.uncor - min(avg.uncor), type = "l",
	xlab = "Time (ms)", ylab = "Amplitude",ylim=c(0,400),
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.55\textwidth,height=0.4\textheight]{figs/fig-uncorrectedCorrected3.pdf}
  \caption{Top left panel: Uncorrected (black) and corrected (red) waveforms at
  channel E21. Bottom left panel: Topographic map of uncorrected blinks. Bottom
...
pdf(file="figs/fig-ICNumTrials5.pdf")
plot(smry$NumTrial, type = "h", xlab="IC", ylab = "Number of Trials", 
	xaxt = "n")
myat<-pretty(1:nrow(smry), 10)
myat[1]<-1
...
\begin{figure}[ht]
  \centering
  \includegraphics{figs/fig-ICNumTrials5.pdf}
  \caption{Number of Trials that correlate at or above threshold for each IC.
...
#
# create plot
pdf(file = "figs/fig-uncorrectedCorrected2.pdf")
par(mfrow = c(2, 2), mar = c(5.1, 4.1, 4.1, 2.1))
# plot waveforms
plot(time, avg.uncor - min(avg.uncor), type = "l",
	xlab = "Time (ms)", ylab = "Amplitude",ylim=c(0,400),
...
#
# create plot
pdf(file = "figs/fig-uncorrectedCorrected5.pdf")
par(mfrow = c(2, 2), mar = c(5.1, 4.1, 4.1, 2.1))
# plot waveforms
plot(time, avg.uncor - min(avg.uncor), type = "l",
	xlab = "Time (ms)", ylab = "Amplitude",ylim=c(0,400),
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.55\textwidth,height=0.4\textheight]{figs/fig-uncorrectedCorrected5.pdf}
  \caption{Top left panel: Uncorrected (black) and corrected (red) waveforms at
  channel E21. Bottom left panel: Topographic map of uncorrected blinks. Bottom
...
pdf(file="figs/fig-ICNumTrials12.pdf")
plot(smry$NumTrial, type = "h", xlab="IC", ylab = "Number of Trials", 
	xaxt = "n")
myat<-pretty(1:nrow(smry), 10)
myat[1]<-1
...
\begin{figure}[ht]
  \centering
  \includegraphics{figs/fig-ICNumTrials12.pdf}
  \caption{Number of Trials that correlate at or above threshold for each IC.
...
#
# create plot
pdf(file = "figs/fig-uncorrectedCorrected12.pdf")
par(mfrow = c(2, 2), mar = c(5.1, 4.1, 4.1, 2.1))
# plot waveforms
plot(time, avg.uncor - min(avg.uncor), type = "l",
	xlab = "Time (ms)", ylab = "Amplitude",ylim=c(0,400),
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.55\textwidth,height=0.4\textheight]{figs/fig-uncorrectedCorrected12.pdf}
  \caption{Top left panel: Uncorrected (black) and corrected (red) waveforms at
  channel E21. Bottom left panel: Topographic map of uncorrected blinks. Bottom
...
pdf(file = "figs/fig-beforeAfter5IC.pdf")
plot.avgba(x = res.up2, data = egi129, channel = c("E17", "E11",
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.55\textwidth,height=0.4\textheight]{figs/fig-beforeAfter5IC.pdf}
...
rownames(dat)<-1:nrow(dat)
# 
pdf(file="fig-eeglab.pdf")
plot.avgba(x = eeglab, data = dat, channel = c("E17", "E11",
	"E16", "E6", "E129", "E55", "E62", "E72", "E75", "E81"), 
	ylim = c(-5, 5), new.page = FALSE)
dev.off()

dev.new()
plot.trba(x = eeglab, data = dat, channel = "E21")

cor(eeglab$data$E21, dat$E21)
# [1] 0.03127444
#
pdf(file="fig-eeglab.pdf")
plot.trba(x = eeglab, data = dat, channel = "E21", new.page = FALSE)
dev.off()
@
}
...
#
# create plot
pdf(file = "figs/fig-EEGLABuncorrectedCorrected.pdf")
par(mfrow = c(2, 2), mar = c(5.1, 4.1, 4.1, 2.1))
# plot waveforms
plot(time, avg.uncor - min(avg.uncor), type = "l",
...
pdf(file = "figs/fig-EEGLABvsICAocularCorrection.pdf")
par(mfrow = c(2, 2), mar = c(5.1, 4.1, 4.1, 2.1))
# plot waveforms
avg.uncor <- tapply(eeglab.uncor$E21, eeglab.uncor$Time, mean)
avg.cor <- tapply(eeglab.cor$E21, eeglab.cor$Time, mean)
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=0.55\textwidth,height=0.4\textheight]{figs/fig-EEGLABvsICAocularCorrection.pdf}
  ...
  Topographic map of {\tt EEGLAB} corrected blinks. Bottom righ panel:
  Topographic map of {\tt icaOcularCorrection} corrected blinks (some trials +
  top 5 ICs completely zerod-out).}
